...
def test_file_lock_prevents_corruption(
    monkeypatch, github_api, mock_global_config
):
...
def test_progress_deduplication(
    ...
    monkeypatch.setattr(
        DownloadManager, "get_or_create_progress", lambda self: mock_progress
    )
    ...
    assert len(remove_task_calls) == 1, f"Expected exactly 1 task removal, got {len(remove_task_calls)}"
    
    # Verify the task was for the correct file
    task_description = add_task_calls[0][1]
    assert "fake.AppImage" in task_description, f"Task description should contain filename: {task_description}"
