...
class TestUpdateCachedProgress:
    ...
    @pytest.mark.asyncio
    async def test_update_cached_progress_from_cache_false(
        self,
        mock_progress_reporter: MagicMock,
    ) -> None:
        """Description must not contain '(cached)' when from_cache=False.

        Regression for #259: when --refresh-cache is used, the progress
        description must not say 'cached'; otherwise the GitHub Releases
        section renders 'Retrieved from cache' even though the data was
        ...
        call_kwargs = mock_progress_reporter.update_task.call_args[1]
        assert "cached" not in call_kwargs["description"].lower()
