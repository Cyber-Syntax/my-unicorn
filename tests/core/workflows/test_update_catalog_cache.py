...
def test_catalog_caching_first_load(update_manager, mock_config_manager):
    ...
    result = update_manager._load_catalog_cached("qownnotes")
...
def test_catalog_caching_cached_load(update_manager, mock_config_manager):
    """Test catalog is only loaded once for multiple accesses."""
    catalog_data = {"github_owner": "test", "github_repo": "app"}
    mock_config_manager.load_catalog.return_value = catalog_data

    # First call - loads from file
    result1 = update_manager._load_catalog_cached("qownnotes")
    assert mock_config_manager.load_catalog.call_count == 1

    # Second call - returns cached value
    result2 = update_manager._load_catalog_cached("qownnotes")
    assert (
...
def test_catalog_caching_different_catalogs(
...
def test_catalog_caching_none_result(update_manager, mock_config_manager):
    ...
    result1 = update_manager._load_catalog_cached("nonexistent")
    assert result1 is None
    assert mock_config_manager.load_catalog.call_count == 1

    # Second call - should use cached None
    result2 = update_manager._load_catalog_cached("nonexistent")
...
def test_catalog_cache_isolated_per_instance(mock_config_manager):
    ...
    manager1._load_catalog_cached("qownnotes")
    assert mock_config_manager.load_catalog.call_count == 1
    ...
    manager2._load_catalog_cached("qownnotes")
    assert mock_config_manager.load_catalog.call_count == 2
