...
@pytest.fixture
def cmd():
    return UpdateAsyncCommand()

@pytest.mark.parametrize("remaining,limit,apps,expected", [
...
def test_check_rate_limits_sufficient(monkeypatch, cmd, remaining, limit, apps, expected):
    # stub rate limit
    monkeypatch.setattr(GitHubAuthManager, 'get_rate_limit_info', lambda: (remaining, limit, None, True))
    can_proceed, filtered, msg = cmd._check_rate_limits([{'name': f"app{i}"} for i in range(len(apps))])
    assert can_proceed is expected
    assert len(filtered) == len(apps)
    assert "Sufficient API rate limits" in msg

@pytest.mark.parametrize("remaining,limit,apps,expected_count", [
...
def test_check_rate_limits_insufficient(monkeypatch, cmd, remaining, limit, apps, expected_count):
    monkeypatch.setattr(GitHubAuthManager, 'get_rate_limit_info', lambda: (remaining, limit, None, False))
    can_proceed, filtered, msg = cmd._check_rate_limits([{'name': f"app{i}"} for i in range(len(apps))])
    assert can_proceed is False
    assert len(filtered) == expected_count
    assert ("Insufficient API rate limits" in msg) or ("Cannot process any apps" in msg)

@pytest.mark.parametrize("batch_mode,user_input,expected", [
    (True, None, True),
    (False, 'y', True),
    (False, 'n', False),
])
...
async def test_update_single_app_async_failure(monkeypatch, cmd):
    ...
    app_data = {'name': 'app1', 'latest': '2.0'}
    success = await cmd._update_single_app_async(app_data, is_batch=False, app_index=1, total_apps=1)
