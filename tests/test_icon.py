...
class TestAppImageIconExtractor:
    ...
    def mock_squashfs_root(self, tmp_path):
        ...
        png_icon.write_bytes(b"mock png data" * 20)  # Make it larger than min size
    ...
    def test_format_scores(self, extractor):
        ...
        assert extractor.FORMAT_SCORES[".svg"] > extractor.FORMAT_SCORES[".png"]
        assert extractor.FORMAT_SCORES[".png"] > extractor.FORMAT_SCORES[".ico"]
        assert extractor.FORMAT_SCORES[".ico"] > extractor.FORMAT_SCORES[".bmp"]
    ...
    def test_select_best_icon_empty_candidates(self, extractor):
        ...
        with pytest.raises(IconExtractionError, match="No icon candidates available"):
    ...
    def test_find_best_icon_returns_highest_scored(self, extractor, mock_squashfs_root):
    ...
    async def test_extract_appimage_success(self, mock_subprocess, extractor, tmp_path):
    ...
    @patch("asyncio.create_subprocess_exec")
    async def test_extract_appimage_failure(self, mock_subprocess, extractor, tmp_path):
        ...
        with pytest.raises(IconExtractionError, match="AppImage extraction failed"):
            await extractor._extract_appimage(appimage_path, tmp_path)
    ...
    async def test_extract_icon_nonexistent_appimage(self, extractor, tmp_path):
...
class TestIconManager:
    ...
    async def test_extract_icon_only_success(self, mock_extract, icon_manager, tmp_path):
