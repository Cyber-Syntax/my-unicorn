...
class TestSandboxCreation:
    ...
    def test_sandbox_creation(self) -> None:
        ...
        with SandboxEnvironment(name="test_sandbox_creation") as sandbox:
            # Temp directory should exist
            assert sandbox.temp_home.exists()
            assert sandbox.temp_home.is_dir()

            # Temp home should be in system temp directory
            temp_home_str = str(sandbox.temp_home)
            assert temp_home_str.startswith(("/tmp", "/var"))

    def test_sandbox_has_config_directory(self) -> None:
        """Test that sandbox has a .config/my-unicorn directory."""
        with SandboxEnvironment(
            name="test_sandbox_has_config_directory"
        ) as sandbox:
            config_dir = sandbox.temp_home / ".config" / "my-unicorn"
    ...
    def test_sandbox_provides_home_env(self) -> None:
        """Test that sandbox provides HOME environment variable."""
        original_home = os.environ.get("HOME")
        try:
            with SandboxEnvironment(
                name="test_sandbox_provides_home_env"
            ) as sandbox:
                ...
                assert sandbox.temp_home is not None
        finally:
            if original_home:
...
class TestConfigCopy:
    ...
    def test_config_copy_handles_missing_source(self) -> None:
        """Test that config copy handles missing source gracefully."""
        with SandboxEnvironment(
            name="test_config_copy_handles_missing_source"
        ) as sandbox:
...
class TestPathRewriting:
    ...
    def test_path_rewriting_updates_directory_section(
        ...
        with SandboxEnvironment(
            name="test_path_rewriting_updates_directory_section"
        ) as sandbox:
...
"""
    ...
    def test_path_rewriting_relative_paths(self) -> None:
        """Test that path rewriting handles existing relative paths."""
        with SandboxEnvironment(
            name="test_path_rewriting_relative_paths"
        ) as sandbox:
            settings_file = (
                sandbox.temp_home / ".config" / "my-unicorn" / "settings.conf"
            )
...
"""
            ...
            assert "~/" not in content or (str(sandbox.temp_home) in content)

    def test_path_rewriting_preserves_non_directory_settings(self) -> None:
        """Test that path rewriting preserves non-directory settings."""
        with SandboxEnvironment(
            name="test_path_rewriting_preserves_non_directory_settings"
        ) as sandbox:
            settings_file = (
                sandbox.temp_home / ".config" / "my-unicorn" / "settings.conf"
            )
