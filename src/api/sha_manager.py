...
class SHAManager:
    ...
    def _try_extract_sha_from_release_body(self) -> bool:
        ...
        logger.info(f"Attempting to extract SHA for {self.appimage_name} from release description...")
                ...
                for line in checksum_lines:
                    ...
                    if len(parts) == 2:
                        ...
                        if f_name.strip() == appimage_base_name: # Exact filename match
                            ...
                            logger.info(f"Found exact filename match in release description checksums: {f_name}")
                            break
                        if appimage_base_name.lower() in f_name.strip().lower() and not best_match_hash: # Substring match as fallback
                             best_match_hash = hash_value.strip()
                             logger.info(f"Found substring match in release description checksums: {f_name}")

                if best_match_hash:
                    if re.fullmatch(r"[0-9a-f]{64}", best_match_hash, re.IGNORECASE): # Validate SHA256
                        self.hash_type = "sha256"
                        self.extracted_hash_from_body = best_match_hash
                        self.sha_name = "extracted_checksum" # Use the specified name
                        self.sha_url = None
                        logger.info(f"Successfully extracted SHA256 hash '{self.extracted_hash_from_body}' for {self.appimage_name} from release body.")
                        return True
                    else:
                        logger.warning(f"Extracted hash '{best_match_hash}' for {self.appimage_name} from body does not look like SHA256.")
                else:
                    logger.info(f"Checksum lines found for {appimage_base_name}, but no specific hash could be isolated from lines: {checksum_lines}")
            else:
                logger.info(f"No checksums found for {appimage_base_name} in the release description.")
    ...
    def find_sha_asset(self, assets: List[Dict]) -> None:
        ...
        if sha_asset:
            ...
            if self._try_extract_sha_from_release_body():
                logger.info(f"Successfully used SHA extracted for {self.appimage_name} from release description.")
                return # SHA found from body, no need for further file-based fallback
            else:
                logger.warning(f"Could not extract SHA for {self.appimage_name} from release description.")
