...
class ReleaseManager:
    ...
    def _fetch_all_releases_data(self, headers: Dict) -> Tuple[bool, Union[Dict[str, Any], str]]:
        ...
        if response.status_code == 200:  # HTTP_OK
            ...
            if not releases:
                logger.info(f"No releases found at all for {self.owner}/{self.repo} via _fetch_all_releases_data.")
            ...
            logger.debug(f"Successfully fetched all releases, using the latest: {latest_release_data.get('tag_name')}")
        ...
        # Rate limit handling is done by the caller (GitHubAPI) which can decide to refresh auth
            ...
            logger.warning(f"GitHub API rate limit exceeded during _fetch_all_releases_data for {self.owner}/{self.repo}.")
    ...
    def get_latest_release_data(self, headers: Dict) -> Tuple[bool, Union[Dict[str, Any], str]]:
        ...
        if response.status_code == 200:  # HTTP_OK
            ...
            logger.debug(f"Successfully fetched latest stable release: {release_data.get('tag_name')}")
        ...
        # Rate limit handling is done by the caller (GitHubAPI)
            ...
            logger.warning(f"GitHub API rate limit exceeded during get_latest_release_data for {self.owner}/{self.repo}.")
            return False, "GitHub API rate limit exceeded." # Specific error for rate limit
        else:
            error_msg = (
